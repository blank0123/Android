...
public class SharedUtil {
    private static com.example.shoppingcart.util.SharedUtil mUtil; // 声明一个共享参数工具类的实例
    private static SharedPreferences mShared; // 声明一个共享参数的实例

    // 通过单例模式获取共享参数工具类的唯一实例
    public static com.example.shoppingcart.util.SharedUtil getIntance(Context ctx) {
        if (mUtil == null) {
            mUtil = new com.example.shoppingcart.util.SharedUtil();
        }
        // 从share.xml中获取共享参数对象
