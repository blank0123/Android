...
public class CartDBHelper extends SQLiteOpenHelper {
    ...
    private static com.example.shoppingcart.database.CartDBHelper mHelper = null; // 数据库帮助器的实例
    ...
    // 利用单例模式获取数据库帮助器的唯一实例
    public static com.example.shoppingcart.database.CartDBHelper getInstance(Context context, int version) {
        if (version > 0 && mHelper == null) {
            mHelper = new com.example.shoppingcart.database.CartDBHelper(context, version);
        } else if (mHelper == null) {
            mHelper = new com.example.shoppingcart.database.CartDBHelper(context);
        }
        return mHelper;
    }
