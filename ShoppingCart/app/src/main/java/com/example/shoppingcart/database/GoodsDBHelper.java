...
public class GoodsDBHelper extends SQLiteOpenHelper {
    ...
    private static com.example.shoppingcart.database.GoodsDBHelper mHelper = null; // 数据库帮助器的实例
    ...
    // 利用单例模式获取数据库帮助器的唯一实例
    public static com.example.shoppingcart.database.GoodsDBHelper getInstance(Context context, int version) {
        if (version > 0 && mHelper == null) {
            mHelper = new com.example.shoppingcart.database.GoodsDBHelper(context, version);
        } else if (mHelper == null) {
            mHelper = new com.example.shoppingcart.database.GoodsDBHelper(context);
        }
        return mHelper;
    }
