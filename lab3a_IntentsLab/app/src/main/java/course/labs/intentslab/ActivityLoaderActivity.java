...
public class ActivityLoaderActivity extends Activity {
	...
	private void startExplicitActivation() {
		...
		// 上面已经定义了		GET_TEXT_REQUEST_CODE = 1
	}
	...
	private void startImplicitActivation() {
		...
		// 上面已经定义了		URL = "http://www.google.com"

		// TODO - Create a chooser intent, for choosing which Activity
		// will carry out the baseIntent
		// (HINT: Use the Intent class' createChooser() method)
		//Intent chooserIntent = Intent.createChooser(baseIntent, "Browser");
         Intent chooserIntent = Intent.createChooser(baseIntent,CHOOSER_TEXT);
		// 上面已经定义了		CHOOSER_TEXT = "Load " + URL + " with:"
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		super.onActivityResult(requestCode,resultCode,data);		//对目标效果无影响
		if (resultCode == RESULT_OK) {						//这里是resultCode
			if (requestCode==1){
				mUserTextView.setText(data.getStringExtra("result"));
//				mUserTextView.setText(data.getCharSequenceExtra("result"));
